 ...
 *
 * The API Gateway validates the JWT and forwards trusted headers:
 *   X-User-Id    → the authenticated user's numeric ID
 *   X-User-Email → the authenticated user's email
 *   X-User-Role  → the user's role (e.g. CUSTOMER, STORE, DELIVERY)
 *   X-Store-Id   → the dark store assigned to this user's location
 *
